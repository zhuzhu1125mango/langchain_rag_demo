...
<#
.SYNOPSIS
    LangChain RAG Demo - View Production Environment Logs

.DESCRIPTION
    PowerShell 7 optimized script to view production environment logs
...
.EXAMPLE
    ...
    .\scripts\logs-prod.ps1 -Service backend
.NOTES
    用法：./scripts/logs-prod.ps1 [[-Service] <服务名>]
#>
...
$script:Services = @("backend", "frontend", "postgres", "minio", "milvus-standalone", "etcd", "redis", "prometheus", "grafana", "alertmanager", "postgres-exporter")
...
Write-Host "${Bold}${Cyan}=== Production Environment Logs ===${Reset}"
Write-Host ""
...
if ($Service) {
    ...
    Write-Host ""
    Write-Host "Tip: Use '.\scripts\logs-prod.ps1 <service>' to view specific service logs"
