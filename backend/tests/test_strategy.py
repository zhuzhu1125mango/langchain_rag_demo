...
class TestKeywordStrategy:
    ...
    def test_greeting_detection(self):
        ...
        for greeting in greetings:
            result = self.strategy.should_use_knowledge_base(greeting)
            assert result is False, f"问候语 '{greeting}' 应该返回 False"
    
    def test_knowledge_question_detection(self):
        """测试知识库相关问题检测"""
        knowledge_questions = [
            "请根据文档内容回答",
            "参考资料中的数据是什么",
        ...
        for question in knowledge_questions:
            result = self.strategy.should_use_knowledge_base(question)
            assert result is True, f"知识库问题 '{question}' 应该返回 True"
    
    def test_confidence_range(self):
        """测试置信度范围"""
        self.strategy.should_use_knowledge_base("你好")
...
class TestContextStrategy:
    ...
    def test_with_history_knowledge_context(self):
        """测试有历史对话上下文（知识库相关）"""
        history = [
        ...
        result = self.strategy.should_use_knowledge_base("请详细说明", history)
        assert result is True, "在知识库上下文后提问应该返回 True"
    
    def test_with_history_greeting_context(self):
        """测试有历史对话上下文（问候相关）"""
        history = [
        ...
        result = self.strategy.should_use_knowledge_base("就是打个招呼", history)
