    ...
    try:
        ...
        try:
            ...
            result = await db.execute(
                select(SessionModel).filter(SessionModel.id == uuid.UUID(data.session_id))
            )
...
async def get_feedback_stats(session_id: Optional[str] = None, db: AsyncSession = Depends(get_db)):
    """
    获取评价统计
    ...
    positive_query = select(func.count(Feedback.id)).filter(Feedback.rating >= 4)
    if where_clause:
        positive_query = positive_query.filter(*where_clause)
    positive_result = await db.execute(positive_query)
    positive_count = positive_result.scalar_one()
    
    negative_query = select(func.count(Feedback.id)).filter(Feedback.rating <= 2)
    if where_clause:
        negative_query = negative_query.filter(*where_clause)
    ...
    if where_clause:
        avg_query = avg_query.filter(*where_clause)
...
@router.post("/like")
async def like_message(message_id: str, session_id: str, db: AsyncSession = Depends(get_db)):
    """
    点赞消息（快捷接口）
...
@router.post("/dislike")
async def dislike_message(message_id: str, session_id: str, db: AsyncSession = Depends(get_db)):
    """
    差评消息（快捷接口）
